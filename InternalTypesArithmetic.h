...
ALWAYS_INLINE F32x2 F32x2Div(F32x2 x, F32x2 y, const int8_t Q)
{
	F32x2 zeroValue = F32x2Zero();
	F32x2 maxValue = F32x2Set(INT32_MAX);
	F32x2 precision = F32x2Set(DIV_PRECISION);

	F32x2 low = zeroValue;				//low boundary
	F32x2 high = maxValue;					//high boundary
	F32x2 mid;								//middle value
	F32x2 midY;								//mid * y

	Boolx2 resultIsNegative = F32x2LessThan(F32x2XOR(x, y), zeroValue);
	...
	Boolx2 yIs0 = F32x2Equal(y, zeroValue);			//if y == 0
	Boolx2 xIs0 = F32x2Equal(x, zeroValue);			//if x == 0
	...
	while (1)
	{
		...
		if ((int8_t)isCalculated == 3)
		{
			F32x2MovIfTrue(mid, F32x2Sub(zeroValue, mid), resultIsNegative);
			return mid;
		}
